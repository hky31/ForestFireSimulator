...
public class FireSimulationService
{
    ...
    public void Step()
    {
        ...
        for (int i = 0; i < SIZE; i++)
        {
            ...
            {
                newForest.Cells[i][j] = forest.Cells[i][j] switch
                {
    ...
    public void InitForest()
    {
        ...
        forest.Cells[SIZE / 2][SIZE / 2] = TreeState.Fire;
        forest.Cells[SIZE / 4][SIZE / 2] = TreeState.Fire;
        forest.Cells[SIZE / 4][SIZE / 4] = TreeState.Fire;
    }
    ...
    private bool HasBurningNeighbor(int x, int y) =>
        (x > 0 && forest.Cells[x - 1][y] == TreeState.Fire) ||
        (x < SIZE - 1 && forest.Cells[x + 1][y] == TreeState.Fire) ||
        (y > 0 && forest.Cells[x][y - 1] == TreeState.Fire) ||
        (y < SIZE - 1 && forest.Cells[x][y + 1] == TreeState.Fire);
}
